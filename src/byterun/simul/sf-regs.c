...
void esp8266_pin_mode(uint8_t p, uint8_t mode) {
...
void esp8266_digital_write(uint8_t p, uint8_t level) {
  if(level == 0) printf("Wrote LOW to pin %d\n", p);
  else printf("Wrote HIGH to pin %d\n", p);
}

uint8_t esp8266_digital_read(uint8_t _p) {
  return 0;
}
